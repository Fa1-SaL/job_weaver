...
export default function TetrisLoading({ 
  ...
  // Size configurations
  const sizeConfig = {
    sm: { cellSize: 'w-2 h-2', gridWidth: 8, gridHeight: 14, padding: 'p-1' },
    md: { cellSize: 'w-3 h-3', gridWidth: 10, gridHeight: 15, padding: 'p-1.5' },
    lg: { cellSize: 'w-4 h-4', gridWidth: 10, gridHeight: 16, padding: 'p-2' }
  }
  ...
  const baseText = (loadingText || "").replace(/\.+$/, "").trim()
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "100%" }}>
      ...
      {showLoadingText && (
        <div className="text-center">
          <p className="text-black dark:text-white font-medium transition-colors" style={{ margin: 0 }}>
            {baseText}{dots}
          </p>
        </div>
      )}
